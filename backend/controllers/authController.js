...
const resetPassword = async (req,res)=>{
    ...
    res.status(200).json({message:"Password update successfully"});
        ...
        console.error("Rest Password error:",error.message);
        res.status(500).json({message:"Something Went Wrong"});
    ...
    try{
        ...
        if(currentPassword || newPassword){
            ...
            if(newPassword.length < 6){
                return res.status(400).json({message:"Password must be altest 6 Characters"});
            }
        ...
        res.staus(500).json({message:"Error in Updating profile"});
...
export { signup, login, me,forgotPassword,resetPassword,editProfile};
